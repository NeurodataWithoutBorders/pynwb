...
class TestImagingPlaneIO(base.TestMapRoundTrip):
    ...
    def setUpContainer(self):
        self.optical_channel = OpticalChannel('optchan1', 'unit test TestImagingPlaneIO', 'a fake OpticalChannel', '3.14')
    ...
    def setUpBuilder(self):
        optchan_builder = GroupBuilder('optchan1',
                            attributes={'neurodata_type': 'OpticalChannel',
                                        'namespace': 'core',
                                        'help': 'Metadata about an optical channel used to record from an imaging plane',
                                        'source': 'unit test TestImagingPlaneIO'},
                            datasets=  {'description': DatasetBuilder('description', 'a fake OpticalChannel'),
                                        'emission_lambda': DatasetBuilder('emission_lambda', '3.14')},
                         )
        return GroupBuilder('imgpln1',
                            attributes={'neurodata_type': 'ImagingPlane',
                                        'namespace': 'core',
                                        'source': 'unit test TestImagingPlaneIO',
                                        'help': 'Metadata about an imaging plane'},
                            datasets=  {'description': DatasetBuilder('description', 'a fake ImagingPlane'),
...
class TestTwoPhotonSeries(base.TestMapRoundTrip):
    ...
    def setUpContainer(self):
        ...
        fov = [2.0, 2.0, 5.0]
        ret = TwoPhotonSeries('test_2ps', 'unit test TestTwoPhotonSeries', data, self.imaging_plane, 'image_unit', 'raw', fov, 1.7, 3.4, timestamps=timestamps, dimension=[2])
        return ret
    ...
    def setUpBuilder(self):
        ...
        imgpln_builder = GroupBuilder('imgpln1',
                            attributes={'neurodata_type': 'ImagingPlane',
                                        'namespace': 'core',
                                        'help': 'Metadata about an imaging plane',
                                        'source': 'unit test TestTwoPhotonSeries'},
        ...
        return GroupBuilder('test_2ps',
                                attributes={'source': 'unit test TestTwoPhotonSeries',
