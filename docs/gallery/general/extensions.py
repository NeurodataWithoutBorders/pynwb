...
The NWB format was designed to be easily extendable. Here we discuss basic functionality
...
to this namespace. Finally, it calls :py:meth:`~hdmf.spec.write.NamespaceBuilder.export` to save the extensions to
disk for downstream use.
"""
...
# .. tip::
#
#     Detailed documentation of all components and `neurodata_types` that are part of the core NWB schema are
...
with NWBHDF5IO("cache_spec_example.nwb", mode="w") as io:
    io.write(nwbfile)
...
ndx_name = "test_multicontainerinterface"
ns_path = ndx_name + ".namespace.yaml"
ext_source = ndx_name + ".extensions.yaml"

ns_builder = NWBNamespaceBuilder(ndx_name + " extensions", ndx_name, version="0.1.0")
...
# generally done in a different file). You can do this manually:
...
@register_class("Potato", ndx_name)
...
@register_class("PotatoSack", ndx_name)
...
####################
# You can also generate these classes automatically.

from pynwb import get_class, load_namespaces

load_namespaces(ns_path)

Potato = get_class("Potato", ndx_name)
PotatoSack = get_class("PotatoSack", ndx_name)
...
nwbfile = NWBFile(
    ...
    identifier="NB123A",
    session_start_time=datetime(2018, 6, 1, tzinfo=tzlocal()),
)
...
with NWBHDF5IO("test_multicontainerinterface.nwb", "r") as io:
    nwb = io.read()
    print(nwb.processing["module_name"]["potato_sack"].get_potato("big_potato").weight)
