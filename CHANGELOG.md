...
### Fixed
- Worked around a deadlock that hung the Windows ROS3 CI jobs after the tests passed. The HDF5 2.1 ROS3 driver deadlocks while the process exits on Windows: during `DLL_PROCESS_DETACH`, `hdf5.dll` runs an `atexit` handler that calls `aws_s3_library_clean_up`, which blocks forever joining AWS worker threads under the loader lock. The ROS3 CI jobs now run through `run_ros3_hang_safe.py`, which executes `test.py` in a child process and terminates it from the parent once the tests report their result, and carry a `timeout-minutes` backstop. @rly [#2228](https://github.com/NeurodataWithoutBorders/pynwb/issues/2228)
