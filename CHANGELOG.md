...
### Fixed
- Fixed `pynwb.read_nwb` leaking the `fsspec` file handle when reading a remote HDF5 file. `NWBHDF5IO.read_nwb` opened the remote object through `fsspec` and passed the handle to `h5py`, which does not close a file object it is given, so the handle stayed open after `get_read_io().close()`. On Windows the open handle held a lock that blocked deletion of the underlying file. The `NWBHDF5IO` now closes the `fsspec` handle when it is closed. @rly [#2226](https://github.com/NeurodataWithoutBorders/pynwb/pull/2226)
