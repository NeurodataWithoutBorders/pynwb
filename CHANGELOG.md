...
### Fixed
- Fixed `FeatureExtraction.times` and `Clustering.peak_over_rms` being copied into a Python list on construction, which read the backing dataset one element at a time. On a recording with 23,630 feature times this made `io.read()` 24 ms slower than a single bulk read, and it undid the lazy-array support these fields gained in [#2235](https://github.com/NeurodataWithoutBorders/pynwb/pull/2235). Both fields are now stored as given, like the `features` and `num` fields beside them. @cboulay [#2251](https://github.com/NeurodataWithoutBorders/pynwb/pull/2251)
