...
## Changed
- Consolidated dependency declarations into `pyproject.toml` using PEP 735 `[dependency-groups]` (`test`, `optional`, `docs`), and removed the `requirements.txt`, `requirements-dev.txt`, `requirements-opt.txt`, `requirements-doc.txt`, and `requirements-min.txt` files. `tox` now installs dependencies via dependency groups, with minimum-version testing using `uv pip install --resolution lowest-direct`. Install development dependencies with `pip install --group test --group docs -e .`. @rly [#2205](https://github.com/NeurodataWithoutBorders/pynwb/pull/2205)
