...
class NWBFile(MultiContainerInterface):
    ...
    def __init__(self, **kwargs):
        ...
        if trials is None:
            trials = DynamicTable('trials', 'autogenerated by PyNWB API')
            trials.add_column('start', 'the start time of each trial')
            trials.add_column('end', 'the end time of each trial')
        self.trials = trials
