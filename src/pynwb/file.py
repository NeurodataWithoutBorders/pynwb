...
class NWBFile(MultiContainerInterface):
    ...
    def __init__(self, **kwargs):
        ...
        if self.__session_start_time.tzinfo is None:
            self.__session_start_time = self.add_missing_timezone(self.__session_start_time)
    ...
    @docval({'name': 'date', 'type': datetime, 'doc': 'datetime object with missing timezone'})
    def add_missing_timezone(self, date):
        """
