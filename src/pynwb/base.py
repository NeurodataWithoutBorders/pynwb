...
class TimeSeries(NWBDataInterface):
    ...
    @docval({'name': 'name', 'type': str, 'doc': 'The name of this TimeSeries dataset'},  # required
            ...
            {'name': 'continuity', 'type': str, 'default': None, 'enum': ["continuous", "instantaneous", "step"],
             'doc': 'Optionally describe the continuity of the data. Can be "continuous" or "step". '
                    'For example, a voltage trace would be "continuous", because samples are recorded from a '
                    'continuous process. Times of image presentations would be "step" because the picture '
                    ...
                    'NOTE: "instantaneous" is deprecated. Use an EventsTable instead for event data.'},
              allow_positional=AllowPositional.WARNING,)
    def __init__(self, **kwargs):
